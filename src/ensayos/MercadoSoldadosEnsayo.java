...
public class MercadoSoldadosEnsayo extends JPanel {
	...
	public MercadoSoldadosEnsayo(Batallon batallon) {
		...
		add(btnOk);
		annadirFocusLost(lblTotal);
	}

	private void annadirFocusLost(JLabel lblTotal) {
		FocusAdapter adaptadorFocus = new FocusAdapter() {
			@Override
		...
		for (EspecialidadSoldadosEnsayo especialidadSoldadosEnsayo : especialidades) {
			especialidadSoldadosEnsayo.getTxtCantidad().addFocusListener(adaptadorFocus);
		}
	}
